...
classdef Robot < OM_X_arm
    ...
    methods
        ...
        % Sends the joints to the desired angles
        % goals [1x4 double] - angles (degrees) for each of the joints to go to
        % travelTime (optional) - desired travel time (milliseconds)
        function set_joint_vars(self, goals, varargin)
            narginchk(2, 3)
            if(~isequal(size(goals), [1 4]))
                throw(MException('Robot:IncorrectInput', 'goals is incorrect size'));
            end
            if(size(varargin) > 0) 
                self.writeTime(varargin{1});
            end
            self.writeJoints(goals);
        end
        function read_joints=read_joint_vars(self, GETPOS, GETVEL)
            read_joints = zeros(2,4);
            if (GETPOS==true)
                read_joints(1, :)=(self.bulkReadWrite(DX_XM430_W350.POS_LEN, DX_XM430_W350.CURR_POSITION) - DX_XM430_W350.TICK_POS_OFFSET) ./ DX_XM430_W350.TICKS_PER_DEG;
            end
            if (GETVEL==true)
                read_joints(2, :)=self.bulkReadWrite(DX_XM430_W350.VEL_LEN, DX_XM430_W350.CURR_VELOCITY) ./ DX_XM430_W350.TICKS_PER_ANGVEL;
            end
        end
    end % end methods
end % end class 